...
export const deleteUsers = async (role, id) => {
  ...
  const response = await $apiAdmin.post(`/api/admin/delete/${id}`);
  return response.data;
};
...
export const editRoleUsers = async (role, id, roleUp) => {
  ...
  const response = await $apiAdmin.patch(`/api/admin/update-role/${id}`, {
    role: roleUp,
  });
...
export const updateUser = async (name, email) => {
  if (name)
    return await $apiAdmin.patch(`/api/admin/update-me`, {
      name: name.trim(),
    });
  if (email)
    return await $apiAdmin.patch(`/api/admin/update-me`, {
...
export const updatePass = async (password, newPass) => {
  const response = await $apiAdmin.patch(`/api/admin/update-pass`, {
    password,
    newPass,
  });
  return response.data;
...
export const createUser = async (name, password, role) => {
  const response = await $apiAdmin.post(`/api/admin/register`, {
